...
// ===== Audio engine =====
const SOUND_MODES = ['underwater', 'forest', 'space', 'machine', 'dream', 'void'];
const soundMode = SOUND_MODES[Math.floor(Math.random() * SOUND_MODES.length)];
const SOUND_NAMES = {
...
function initAudio() {
  ...
  droneOsc.type = soundMode === 'machine' ? 'sawtooth' : 'sine';
  droneOsc.frequency.value = soundMode === 'space' ? 28 : 55;
  ...
  if (soundMode !== 'void') {
    ...
    nf.type = soundMode === 'underwater' ? 'lowpass' : 'bandpass';
    nf.frequency.value = soundMode === 'underwater' ? 280 : 650;
    nsrc.connect(nf);
    nf.connect(masterGain);
    nsrc.start();
  }

  masterGain.gain.linearRampToValueAtTime(currentVolume, t + 1.2);
}
...
function playExplosionSound() {
  if (!audioCtx || !masterGain) return;
  const len = Math.floor(audioCtx.sampleRate * 0.32);
  const buf = audioCtx.createBuffer(1, len, audioCtx.sampleRate);
  const data = buf.getChannelData(0);
  for (let i = 0; i < len; i++) {
...
function modulateDrone(freq) {
  ...
  droneOsc.frequency.linearRampToValueAtTime(freq, audioCtx.currentTime + 0.08);
}
...
function toggleMute() {
  ...
  muteBtn.textContent = isMuted ? '▶ ЗВУК' : '◼ ЗВУК';
  muteBtn.classList.toggle('muted', isMuted);
}

function showVolumeControl() {
  volWrap.style.opacity = '0.4';
}

console.log(`%cСАУНДСКЕЙП: ${SOUND_NAMES[soundMode]}`, 'color:#ffaa00; font-size:12px; font-family:monospace');
